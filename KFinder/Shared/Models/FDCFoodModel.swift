...
class FDCFoodModel: ObservableObject {
  private let service = FoodDataCentralService()
  private let kNutrientNumberRegEx = /428|429|430/
  
  @Published var food: SurveyFoodItem
  
  init(food: SurveyFoodItem = SurveyFoodItem.empty) {
    self.food = food
  }
  
  @MainActor
  func load(with id: Int) async {
    self.food = await service.getFood(with: id)
  }
  
  func findVitaminK() -> [FoodNutrient] {
    guard let foodNutrients = food.foodNutrients else {
      return []
    }
    return foodNutrients.filter {
      try! kNutrientNumberRegEx.firstMatch(in: $0.nutrient.number) != nil
    }
  }
  
  func getAllButVitaminK() -> [FoodNutrient] {
    guard let foodNutrients = food.foodNutrients else {
      return []
    }
    return foodNutrients.filter {
...
struct SurveyFoodItem: Codable {
  ...
  init(fdcId: Int, dataType: String, description: String, foodNutrients: [FoodNutrient]? = nil, foodClass: String? = nil, foodCode: String? = nil, publicationDate: String? = nil, foodCategory: FoodCategory? = nil, foodPortions: [FoodPortion]? = nil) {
    self.fdcId = fdcId
